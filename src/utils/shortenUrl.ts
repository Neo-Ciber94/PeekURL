...
export async function shortenUrl({ url, size, keepDomain = true, keepTld = false }: ShortenUrlOptions): Promise<string> {
    // Expected an output like: 
    // https://www.google.com -> /google/0dAZpLlDvU9l07cN

    if (keepTld === true && keepDomain === false) {
        throw new Error("You should keep the domain name if want to show the top-level-domain");
    }
